...
**Plan**: Planung der Analyse, z.B. Operationalisierung/ Variablenauswahl. Wahl des Stichprobenverfahren und/ oder Versuchsplanung.
...
**Hinweis**: Diese Data Literacy Slidecast dienen didaktisch der konzeptionell-methodischen Verdeutlichung, nicht der inhaltlichen Analyse.
...
`csv` Datendatei (`Datenbasis_SATS_BFI.csv`) in als `R` einlesen und dort `SATS_BFI` nennen:
...
- `r ncol(SATS_BFI)` Variablen:
  ...
  - `D_BFI_G`: Operationalsisierung von *Gewissenhaftigkeit* (als intervallskaliert aufgefasst)
