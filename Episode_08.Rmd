...
Wir hätten andere geschätzte Koeffizienten $\hat{\beta_0}, \hat{\beta}_1$ gehabt als die, die wir haben.
...
**Idee**: Simuliere zufälliges Ziehen einer Stichprobe aus einer Population durch Ziehen mit Zurücklegen aus der Stichprobe.
...
Das $95\%$-Konfidenzintervall für $\beta_{\text{Eingeschätzte Kompetenz}}$ überdeckt den Bereich $`r floor(confint(Bootvtlg$B_SATS_Kompetenz)[1]*100)/100`$ bis $`r ceiling(confint(Bootvtlg$B_SATS_Kompetenz)[2]*100)/100`$. Werte in diesem Bereich sind mit den Daten *kompatibel*.
