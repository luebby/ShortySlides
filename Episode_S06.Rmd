...
Modelliere die .violet[abbhängige Variable] `total.bill`, $\color{violet} y$,  auf Basis der .olive[unabhängigen Variablen] `time` und `size`, $\color{olive}{x_1, x_2}$:
...
Die **Anwendungsdaten** (`tips_anwendung`) enthalten nur die unabhänigen Variablen $\color{olive}{x_1, x_2}$, die abhängige Variable $\color{violet} y$ nicht:
...
Zuvor gelerntes Modell (`erg_lm`) auf Basis der Trainingsdaten (`tips_train`) zur Prognose der .violet[Zielvariable] `total_bill` auf den Anwendungsdaten verwenden:
...
$$\color{purple}{\widehat{\text{total_bill}}_1}=`r round(coef(erg_lm)[1],2)` `r round(coef(erg_lm)[2],4)`\cdot\color{olive}{0} + `r round(coef(erg_lm)[3],2)` \cdot \color{olive}{6}=\color{purple}{`r round(lm_predict[1],2)`}$$
